...
class SVM():
    ...
    def plotModel2D(self,X=[],Y=[],spptInds=[],xLabel="",yLabel="",title="",fName="",isLinePlot=False):
        ...
        plt.plot(X[Y[:,0]==-1,0],X[Y[:,0]==-1,1],'cx',label="-1")
        plt.plot(X[Y[:,0]== 1,0],X[Y[:,0]== 1,1],'m.',label="+1")
        ...
        # サポートベクトルのプロット
        if len(spptInds):
            plt.plot(X[spptInds,0],X[spptInds,1],'o',color='none',markeredgecolor='r',markeredgewidth=3,label="support vector")
